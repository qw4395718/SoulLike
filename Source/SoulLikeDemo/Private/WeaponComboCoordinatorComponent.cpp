...
float UWeaponComboCoordinatorComponent::GetAttackStateCost(int type,int CostType)
{
	...
	if (ComboCoordinatorInfoMap.Find(EWeaponModeTyoe(type)))
	{
		...
		if (Info.ComboStateCostMultiplier.Find(Info.CurrentComboNum))
		{
			...
			switch (EWeaponModeTyoe(CostType))
			{
			case EWeaponActionCostType::EWeaponAction_Health :{CostValue = AbilityCostInfo.HealthCost;};break;
			case EWeaponActionCostType::EWeaponAction_Stamina :{CostValue = AbilityCostInfo.StaminaCost;};break;
			case EWeaponActionCostType::EWeaponAction_Magic : {CostValue = AbilityCostInfo.MagicCost;}; break;
			default:break;
			}
			return CostValue;
		}
